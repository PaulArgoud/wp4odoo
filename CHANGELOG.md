...
- **SupportCandy module** — Same bidirectional helpdesk sync as Awesome Support but for SupportCandy. Custom table data access via `$wpdb` (`wpsc_ticket`, `wpsc_ticketmeta`). Detection via `STARTER_STARTER_VERSION`. Exclusive group `helpdesk` (priority 15 — Awesome Support wins). 43 new tests
