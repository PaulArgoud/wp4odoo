...
- 181 new/updated unit tests (2023 total, 3149 assertions): WC Bookings module + handler, pricelist handler, shipment handler, transient cache isolation, nullable entity_id, memory threshold, stale timeout configuration, schema cache, reconciler, HMAC signature, pull coordinator, WPML adapter (12 tests), Polylang adapter (12 tests), Translation_Service (20 tests), Odoo_Client context (7 tests)
